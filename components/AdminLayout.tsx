...
interface AdminLayoutProps {
  active: 'orders' | 'users' | 'food' | 'integration' | 'payments' | 'delivery';
...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ active, children }) => {
  ...
  const navItemClasses = (isActive: boolean) =>
    `flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${isActive
      ...
      <aside className="w-64 bg-gray-900 text-white flex flex-col">
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          ...
          {canManageDelivery && (
            <a
              href="/admin/delivery"
              className={navItemClasses(active === 'delivery')}
            >
        ...
        <div className="px-4 py-3 border-t border-gray-800 text-xs text-gray-500">
          <span>© {new Date().getFullYear()} Zakątek Smaków</span>
        </div>
      </aside>
